...
class OfflineTestCaseMixin:
    ...
    def setUp(self):
        ...
        # We've hardcoded TEMPLATES[0] to be Django user backend and
        # TEMPLATES[1] to be Jinja2 user backend in test_settings.
...
class OfflineCompressStaticUrlIndependenceTestCase(
    ...
    I.e. user can use the manifest with any other STATIC_URL in the future.
    """
...
class OfflineCompressTestCaseErrors(OfflineTestCaseMixin, TestCase):
    ...
    def _test_offline(self, engine, verbosity=0):
        ...
        if engine == "django":
            ...
            # actually load the 'extends' and 'includes' user, and so
...
class OfflineCompressBlockSuperBaseCompressed(OfflineTestCaseMixin, TestCase):
    template_names = ["_base.html", "base2.html", "test_compressor_offline.html"]
...
class OfflineCompressExtendsRecursionTestCase(OfflineTestCaseMixin, TestCase):
    """
    Test that user extending user with the same name
...
class OfflineCompressExtendsRelativeTestCase(
    ...
    """
    Test that user extending user using relative paths
    (e.g. ./_base.html) are evaluated correctly
    """
